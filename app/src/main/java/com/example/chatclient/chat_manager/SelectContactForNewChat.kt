...
class SelectContactForNewChat : AppCompatActivity() {
    ...
    companion object {
        ...
        const val INTENT_EXTRA_KEY = "user name of user selected for new private chat"
    }
    ...
    private fun createListener(adapter: GroupAdapter<ViewHolder>): IncomingMessageListener {
        ...
        return object : IncomingMessageListener {
            ...
            override fun incomingMessage(text: String) {
                ...
                if (text == currentUser!!.displayName || text.contains("@") || text.contains("from $userSelected") || text.startsWith(
                        Config.FAKE_USERNAME_PREFIX
                    )
                ) {
